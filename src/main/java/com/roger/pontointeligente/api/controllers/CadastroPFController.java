...
import com.roger.pontointeligente.api.dtos.CadastroPFDto;
...
public class CadastroPFController {
	...
	public ResponseEntity<Response<CadastroPFDto>> cadastrar(@Valid @RequestBody CadastroPFDto cadastroPFDto, BindingResult result) throws NoSuchAlgorithmException {
		...
		if (result.hasErrors()) {
			...
			return ResponseEntity.badRequest().body(response);  //Erro 400 do http
		}
	...
	private void validarDadosExistentes(CadastroPFDto cadastroPFDto, BindingResult result) {
		...
		this.funcionarioService.buscaPorCPf(cadastroPFDto.getCpf()).ifPresent(func -> result.addError(new ObjectError("funcionario", "Funcionário já existente")));
		
		this.funcionarioService.buscaPorEmail(cadastroPFDto.getEmail()).ifPresent(func -> result.addError(new ObjectError("funcionario", "Email já existente")));
	}
	...
	private Funcionario converterDtoParaFuncionario(CadastroPFDto cadastroPFDto, BindingResult result) throws NoSuchAlgorithmException {
		...
		//Atributos não obrigatórios por isso o uso de ifPresente()
	...
	/**
	 * popula o DTO de cadastro com os dados do Funcionário e Empresa (com ID) para retorno
	 * 
	 * @param funcionario
